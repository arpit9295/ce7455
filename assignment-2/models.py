...
from utils import init_embedding, init_lstm, init_linear, log_sum_exp, argmax
from data import START_TAG, STOP_TAG
...
class BiLSTM_CRF(nn.Module):
    ...
    def __init__(self,
        ...
        if char_embedding_dim is not None:
            ...
            if self.char_mode == 'LSTM':
                self.char_lstm = nn.LSTM(
                    char_embedding_dim, self.char_lstm_dim, num_layers=1, bidirectional=True)
        ...
        if self.encoder_mode == 'LSTM':
            if self.char_mode == 'LSTM':
                self.lstm = nn.LSTM(
                    embedding_dim+self.char_lstm_dim*2, hidden_dim, bidirectional=True)
            if self.char_mode == 'CNN':
                self.lstm = nn.LSTM(
        ...
        if self.encoder_mode == 'CNN':
            ...
            self.conv1 = nn.Conv2d(
                in_channels=1, out_channels=hidden_dim*2, kernel_size=(1, 1), padding=(0, 0))
        ...
        if self.encoder_mode == 'CNN2':
            ...
            self.conv2 = nn.Conv2d(
                in_channels=hidden_dim*2, out_channels=hidden_dim*2, kernel_size=(1, 1), padding=(0, 0))
        ...
        if self.encoder_mode == 'CNN_DILATED':
            # Conv layer
            self.conv1 = nn.Conv2d(in_channels=1, out_channels=hidden_dim*2,
                                   kernel_size=(1, 2), padding=(0, 0), dilation=1)
            self.conv2 = nn.Conv2d(in_channels=hidden_dim*2, out_channels=hidden_dim *
                                   2, kernel_size=(1, 2), padding=(0, 0), dilation=2)
            self.conv3 = nn.Conv2d(in_channels=hidden_dim*2, out_channels=hidden_dim *
                                   2, kernel_size=(1, 2), padding=(0, 0), dilation=3)
    ...
    def _get_cnn_features(self, sentence, chars2, chars2_length, d):
        ...
        if self.encoder_mode == 'CNN3' or self.encoder_mode == 'CNN_DILATED':
            cnn_out = self.conv2(cnn_out)
            # print(f'conv2: {cnn_out.size()}')
            cnn_out = self.maxpool2(cnn_out)
            # print(f'maxpool2: {cnn_out.size()}')
